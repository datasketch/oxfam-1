...
server <-  function(input, output, session) {
  ...
  get_basic_lang_data <- reactive({
    ...
    if(lang()=="en"){
      ...
      #TODO translate ro apply - and function
      var <- slug_translate |> filter(slug_en %in% input$Indicator)  |> select(slug)

      # for(i  in 1:length(var$slug)){
      #   t  <- as.data.frame(oxfam_one$en[as.vector(var$slug[i])])
      #   colnames(t) <-  c("id", "slug", "slug_en","fecha", "pais","valor","Unidad")
      #   temp <- rbind(temp,t)
      # }


      temp <- plyr::ldply( 1:length(var$slug), function(i){
        t  <- as.data.frame(oxfam_one$en[as.vector(var$slug[i])])
        colnames(t) <-  c("id", "slug", "slug_en","fecha", "pais","valor","Unidad")
        temp <- rbind(temp,t)
      })
      ...
      if(lang()=="es"){
        var <- slug_translate |> filter(slug_es %in% input$Indicator)  |> select(slug)


        # for(i  in 1:length(var$slug)){
        #   t  <- as.data.frame(oxfam_one$es[as.vector(var$slug[i])])
        #   colnames(t) <-  c("id", "slug", "slug_en","fecha", "pais","valor","Unidad")
        #   temp <- rbind(temp,t)
        # }

        temp <- lapply( 1:length(var$slug), function(i){
          t  <- as.data.frame(oxfam_one$es[as.vector(var$slug[i])])
          colnames(t) <-  c("id", "slug", "slug_en","fecha", "pais","valor","Unidad")
          temp <- rbind(temp,t)
        })
        ...
        if(lang()=="pt"){
          ...
          # for(i  in 1:length(var$slug)){
          #   t  <- as.data.frame(oxfam_one$pt[as.vector(var$slug[i])])
          #   colnames(t) <-  c("id", "slug", "slug_en","fecha", "pais","valor","Unidad")
          #   temp <- rbind(temp,t)
          # }
          #
  ...
  sel_country <-reactive({
    if(lang() == "es") {
      unique(oxfam_one$es$new_vaccinations$pais)

    } else {
      if(lang() == "en") {
        unique(oxfam_one$en$new_vaccinations$pais)
      }
      else { if(lang() == "pt")
        unique(oxfam_one$pt$new_vaccinations$pais)
      }
    }
  ...
  sel_indicators <-reactive({
    if(lang() == "es") {
      unique(slug_translate |> filter(slug %in% slug_agg_one$slug)  |> select(slug_es) )
    } else {
      if(lang() == "en") {
        unique(slug_translate |> filter(slug %in% slug_agg_one$slug)  |> select(slug_en) )
      }
      else { if(lang() == "pt")
        unique(slug_translate |> filter(slug %in% slug_agg_one$slug)  |> select(slug_pt) )
      }
    }
  })
  ...
  output$data_range <- renderUI({
    ...
    shinyinvoer::dateRangeInput(inputId='data_range',
                                label = NULL,
                                start = data_range_start(), #", #"31/11/2023",
                                end =  data_range_end(), #"31/12/2023",
                                min = data_range_start(), #"1/01/2022",
                                max = data_range_end(), #"31/12/2024",
                                startLabel = data_range_label_min(),
                                endLabel = data_range_label_max(),
                                resetLabel = NULL,
                                locale = NULL)
  })

  data_range_start <- reactive({
    req(input$Indicator[1])
    req(get_basic_lang_data())
  ...
  data_range_label_min <- reactive({
    i_("date_start", lang= lang())


  })

  data_range_label_max <- reactive({

    i_("date_end", lang= lang())
  ...
  operations <-reactive({
    ...
    if(lang() == "es") {
      var <- slug_translate |> filter(slug_es == input$Indicator[1])  |> select(slug)
    } else {
      if(lang() == "en") {
        var <- slug_translate |> filter(slug_en == input$Indicator[1])  |> select(slug)

      }
      else { if(lang() == "pt")
        var <- slug_translate |> filter(slug_pt == input$Indicator[1])  |> select(slug)
      }
    }

    temp_col <- slug_agg_one |> filter(stringr::str_trim(slug) == stringr::str_trim(var)) |> select(agg)


    i_(as.vector(unique(unlist(stringr::str_split(temp_col$agg, ",")))),lang=lang())
  })
  ...
  indicator_label <- reactive({
    i_(c("slug_comparisons"), lang = lang())
  })


  country_url_r <- reactive({
  ...
  indicator_url_r <- reactive({

    query <- parseQueryString(session$clientData$url_search)
    temp = stringr::str_to_lower(query[["slug"]])
    if(lang() == "es") {
      unique(slug_translate |> filter(slug %in% temp)  |> select(slug_es) )
    } else {
      if(lang() == "en") {
        unique(slug_translate |> filter(slug %in% temp)  |> select(slug_en) )
      }
      else { if(lang() == "pt")
        unique(slug_translate |> filter(slug %in% temp)  |> select(slug_pt) )
      }
    }
  })
  ...
  data_prep <- reactive({
    if(is.null(input$Indicator)) return()
    if(is.null(input$data_range)) return()
